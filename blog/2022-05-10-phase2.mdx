...
<Highlight color="#FFEC1F">
  On Tuesday, May 10th 2022 at 12pm PST, we’re launching Phase 2 of the Iron
  Fish Incentivized Testnet!
</Highlight>
...
Phase 1 token distribution will happen at the same time as Phase 2 token distribution and closer to mainnet. When mainnet? When all the engineering tasks on our [roadmap](https://ironfish.network/roadmap) are complete. If you want this to go faster, consider applying on our [careers page](https://ironfish.network/careers/) for a full time engineering job at Iron FIsh :) or help us with open source contribution as part of the incentivized testnet program.
...
Running an Iron Fish full node with telemetry on and a registered graffiti set gets you max of 10 points for every 12 consecutive hours of hosting (even if running multiple nodes). Points will be automatically assigned to you on the leaderboard. To make sure your node is properly set up to earn points, we made a custom command `ironfish testnet`. Go ahead and run that command before starting your node to make sure you’re all set.
...
Iron Fish is in heavy development. Not only are we focused on the bigger roadmap items for engineerings, but we’re constantly working on improving user experience. Contributing to the ironfish repo is immensely appreciated as it directly impacts the project getting to mainnet faster.

We appreciate open source contributions so much that we are allocating a <b>_separate pool of coins_</b> just for PR contributions. Open source developers no longer have to compete with miners for points in the same pool.
...
We’re dedicating a [Discord channel](https://discord.gg/wrrfFKAhy2) (#community-dev) dedicated purely for open source contribution discussion.

## When will Phase 2 end?

Phase 2 will end when all of the engineering tasks on our [roadmap](https://ironfish.network/roadmap) are completed. Help us get there faster!!! And consider applying or referring someone to our [careers page](https://ironfish.network/careers/).

...
- Find Ironfish and/or Ironfish DKG in the app catalog.
...
After importing the Ledger account into the Node App, you can find the public address for that wallet either by clicking on the public address below account name to save it to your clipboard, navigating to walelt details or by going to the Receive tab:
...
## Multi-sig Wallet Ledger Flow

### Requirements

For the Multi-signer Ledger flow you’ll need the [Node App](https://ironfish.network/use/node-app) as well as the Iron Fish CLI. The easiest way to install the Iron Fish CLI is through NPM. If you already have NodeJS on your computer, you can install Iron Fish CLI by simply running the command `npm install -g ironfish`. For full instructions, please reference the [installation docs](https://ironfish.network/use/get-started/installation).
...
`ironfish wallet:multisig:dkg:create --name dkg-bob -c tcp://c1b4d640-e55a-4d95-8fda-3adf0a8f38fe:Beanss@multisig.ironfish.network:9035`
...
`ironfish wallet:multisig:dkg:create --name ledger-dkg --ledger -c tcp://c1b4d640-e55a-4d95-8fda-3adf0a8f38fe:Beanss@multisig.ironfish.network:9035`

- the `--name` allows you to give this wallet a human readable name or label

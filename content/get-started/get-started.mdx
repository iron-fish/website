...
This guide is intended for crypto novices to understand what Iron Fish is and how they can get started using it. If you want to skip directly to installing and running Iron Fish, you can [install a node](/use/get-started/installation) now.

### Why use $IRON?

Currencies like $IRON promote decentralization by creating a global ledger that no single person can modify. When you run Iron Fish, you help keep this ledger secure by checking that transactions are following the balances in the ledger. By running Iron Fish, you help keep your and your communities coins secure.

The advantage of $IRON is that it's fundamentally private. Other people cannot see the amount sent, or the recipient without your permission. That's thanks to technology called Zero Knowledge Proofs. This is different from Bit Coin which is fundamentally public.

### How can I use $IRON?

There are three way Fish network. Choosing which one is the right solution for you depends on your goals and technical skill.
...
Mining means taking transactions, and adding them to blocks. When mining is done by many different people all over the world, it's one of the main ways Iron Fish remains decentralized and secure. Miners can receive receive $IRON as a reward.
...
Running a node is an important way to contribute to the network. Like a wallet, running a node allows you to send and receive $IRON. Your node will also help validate other peoples transactions to make sure they are obeying the rules that govern all transactions. The more people that are watching to ensure transactions are following the rules, the more secure the network.
 ...
 - Participate - Your node will validate and cast a vote on every single transaction that enters the network. You can help filter our malicious transactions for everyone and yourself.

[Install and run a node](/use/get-started/installation).

...
> **Note:** Once you start a node on a certain network, that node cannot be used on another network. Delete the data directory in `ironfish browse` to reset the data directory.
...
There is no, Devnet faucet. You can instantly mine a block using the built in CPU miner
...
All Iron Fish CLI, Node App, and SDK automatically connect to the main net by default. You don't need to do anything special to use it.

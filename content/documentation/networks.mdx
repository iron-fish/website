...
> **Note:** Once you start a node on a certain network, that node cannot be used on another network. We recommend using a different data directory for each network. Use `ironfish reset --networkId ...` to switch a node to another network.
...
**Testnet Faucet**: You can request Testnet funds by using the faucet. First start your node in the Testnet, then request funds.

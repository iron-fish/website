...
| ERROR        | An error occurred while running the node. Run `ironfish logs -v` to see the latest error messages             |
...
By default, the node stores its data in the `~/.ironfish` directory (see [node data dir](/developers/documentation/node-config)).

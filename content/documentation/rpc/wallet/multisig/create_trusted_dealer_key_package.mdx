...
Creates the key package for a trusted dealer account for the trusted dealer keg generation multi signature signing process. The result also includes the account import for every participant. The account can be imported using the RPC [wallet/importAccount](/developers/documentation/rpc/wallet/import_account).

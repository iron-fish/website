---
title: wallet/multisig/createTrustedDealerPackage
description: RPC Wallet/Multisig | Iron Fish Documentation
---

Defines the structure of a multisig account. The result is a set of accounts to import by each participant using the RPC [wallet/importAccount](https://ironfish.network/developers/documentation/rpc/wallet/import_account). It is the responsibility of the dealer to securely distribute the accounts to the participants.

**Note:** The account imports are encrypted using the participant's identity. This means that the dealer is not able to spend funds from the multisig account. Only the participants are able to import their own account. 

#### Request

```ts
{
  minSigners: number
  participants: Array<{ identity: string }>
}
```

#### Response

```ts
{
  publicAddress: string
  publicKeyPackage: string
  viewKey: string
  incomingViewKey: string
  outgoingViewKey: string
  proofAuthorizingKey: string
  participantAccounts: Array<{ identity: string; account: string }>
}
```

#### Example

The following request body creates a 2-of-3 multisig account:

```json
{
  "minSigners": 2,
  "participants": [
    {
      "identity": ""
    },
    {
      "identity": ""
    },
    {
      "identity": ""
    }
  ]
}
```

Participant identities can be created using the RPC wallet/createParticipant.

###### [View on Github](https://github.com/iron-fish/ironfish/blob/master/ironfish/src/rpc/routes/wallet/getBalance.ts)

---
title: wallet/multisig/createTrustedDealerKeyPackage
description: RPC Wallet/Multisig | Iron Fish Documentation
---

Creates the key package for a trusted dealer account for the trusted dealer keg generation multi signature signing process. The result includes: 

1. The information to import the trusted dealer key package and create the dealer account. 
2. The account import information for every participant.

The accounts can be imported using the RPC [wallet/importAccount](/developers/documentation/rpc/wallet/import_account).

**Note:** The participant account information is encrypted using the participant's identity. This means that the dealer will be unable to import all accounts and spend the funds in the multi signature account. Only the participants are able to import their own account. 

#### Request

```ts
{
  minSigners: number
  participants: Array<{ identity: string }>
}
```

#### Response

```ts
{
  publicAddress: string
  publicKeyPackage: string
  viewKey: string
  incomingViewKey: string
  outgoingViewKey: string
  proofAuthorizingKey: string
  participantAccounts: Array<{ identity: string; account: string }>
}
```

#### Example

The following request body creates a 2-of-3 multisig account:

```json
{
  "minSigners": 2,
  "participants": [
    {
      "identity": ""
    },
    {
      "identity": ""
    },
    {
      "identity": ""
    }
  ]
}
```

Participant identities can be created using the RPC wallet/createParticipant.

###### [View on Github](https://github.com/iron-fish/ironfish/blob/master/ironfish/src/rpc/routes/wallet/multisig/createTrustedDealerKeyPackage.ts)

...
Iron Fish is a privacy-first cryptocurrency that uses zero-knowledge proofs to provide fully private transactions. This means that the Iron Fish network does not publicly display any information about the sender, receiver, or amount of a transaction. The only person who has access to this data is the person who has the private keys for the account of interest. This is very different than other blockchains like Ethereum and Bitcoin where anyone can see this data for any account. With the current architecture, it also means that you need to run a full node to privately interact with the network.

This makes it difficult to use Iron Fish on a mobile device, browser, or any low power machine. In order to use Iron Fish on these devices, we need to be able to build a "light client".

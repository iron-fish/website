...
| $$cv$$      |                                                                                             value commitment                                                                                             |
| $$cm$$      |                                                                                             note commitment                                                                                              |
| $$epk$$     |                                         ephemeral public key (more on this in [Note Encryption and Decryption](#note-encryption-and-decryption))                                         |
| $$C^\{enc\}$$ |                                                                                     encrypted plaintext of the note                                                                                      |
| $$C^\{out\}$$ | encrypted blob that allows the holder of the viewing key to decrypt a decryption key for $$C^\{enc\}$$ (more on this in [Note Encryption and Decryption](#note-encryption-and-decryption)) |
...
| $$esk$$  | ephemeral private key — a random number chosen by the sender (more on this in the [Note Encryption and Decryption section](#note-encryption-and-decryption) |
...
- **Spends**: the list of [Spend Descriptions](#spend-description)
- **Outputs**: the list of [Output Descriptions](#output-description)
- **Binding Signature**: a binding signature that both signs the transaction and is used to [verify](#transaction-verification) that it [balances](#transaction-balancing)
...
1. Verify all the zero-knowledge proofs against the public parameters from the [Spend description](#spend-description)
2. Verify all the zero-knowledge proofs against the public parameters from the [Output description](#output-description)
3. Check that the [transaction balances](#transaction-balancing)

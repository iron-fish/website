...
The Iron Fish network has a set of rules that establish how blocks are accepted to the canonical chain, and what transactions are allowed (beyond the constraints already described in Mining and [Transactions](https://www.notion.so/Transactions-0d0b05511fb8401881d6493fe30b57db?pvs=21)). These rules, collectively, are called **consensus**. Every Iron Fish participant must obey the consensus in order to successfully push transactions or blocks to the Iron Fish chain.
...
|  | Description | Introduced | Mainnet Value | Testnet Value |
| --- | --- | --- | --- | --- |
...
| enableAssetOwnership | Whether ownership of assets can be transferred between accounts (see Changing asset ownership (https://www.notion.so/Changing-asset-ownership-bc791db69e844230b6eadbedaf172fb4?pvs=21)) | Hard Fork 1 [link to FIP] | TBD | TBD |
| enforceSequentialBlockTime | Whether block timestamps must be strictly monotonic (non-decreasing) | Hard Fork 1 [link to FIP] | TBD | TBD |
